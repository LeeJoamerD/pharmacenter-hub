-- Mise à jour de la RPC search_lots_paginated pour inclure les produits détail
          ...
          'produit', json_build_object(
            ...
            'produit_detail', (
              SELECT COALESCE(json_agg(json_build_object(
                'id', pd.id,
                'libelle_produit', pd.libelle_produit,
                'quantite_unites_details_source', pd.quantite_unites_details_source
              )), '[]'::json)
              FROM produits pd 
              WHERE pd.id_produit_source = sl.produit_id
            )
          )
        )
      ) FROM sorted_lots sl
