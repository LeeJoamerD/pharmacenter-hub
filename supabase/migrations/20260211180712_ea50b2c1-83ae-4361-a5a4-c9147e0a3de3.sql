-- Ajouter niveau_detail et has_detail_product à get_pos_products
...
BEGIN
  ...
  FROM (
    SELECT 
      ...
      -- Nouveaux champs pour la mise en détail
      COALESCE(p.niveau_detail, 1) AS niveau_detail,
      EXISTS(
        SELECT 1 FROM produits enfant
        WHERE enfant.id_produit_source = p.id
          AND enfant.is_active = true
          AND COALESCE(enfant.quantite_unites_details_source, 0) > 0
      ) AS has_detail_product
    FROM produits p
