-- Correction: Utiliser l.fournisseur_id au lieu de p.fournisseur_id
DROP FUNCTION IF EXISTS init_inventaire_items(UUID, UUID);

CREATE OR REPLACE FUNCTION init_inventaire_items(
  p_session_id UUID,
  p_tenant_id UUID DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_tenant_id UUID;
  v_session RECORD;
  v_items_count INTEGER := 0;
  v_type TEXT;
  v_filtres_rayon UUID[];
  v_filtres_fournisseur UUID[];
  v_filtres_emplacement TEXT[];
  v_filtres_peremption_jours INTEGER;
  v_cyclique_jours INTEGER;
BEGIN
  -- Récupérer le tenant_id
  IF p_tenant_id IS NOT NULL THEN
    v_tenant_id := p_tenant_id;
  ELSE
    SELECT tenant_id INTO v_tenant_id
    FROM public.personnel
    WHERE auth_user_id = auth.uid();
  END IF;
  
  IF v_tenant_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Tenant non trouvé');
  END IF;
  
  -- Récupérer les informations de la session
  SELECT * INTO v_session
  FROM public.inventaire_sessions
  WHERE id = p_session_id AND tenant_id = v_tenant_id;
  
  IF v_session IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Session non trouvée');
  END IF;
  
  -- Supprimer les items existants pour cette session
  DELETE FROM public.inventaire_items WHERE session_id = p_session_id;
  
  -- Extraire les paramètres de la session
  v_type := COALESCE(v_session.type, 'complet');
  v_filtres_rayon := v_session.filtres_rayon;
  v_filtres_fournisseur := v_session.filtres_fournisseur;
  v_filtres_emplacement := v_session.filtres_emplacement;
  v_filtres_peremption_jours := COALESCE(v_session.filtres_peremption_jours, 90);
  v_cyclique_jours := COALESCE(v_session.cyclique_jours, 30);
  
  -- Insérer les items (type complet, partiel, tournant)
  IF v_type IN ('complet', 'partiel', 'tournant') THEN
    INSERT INTO public.inventaire_items (
      tenant_id, session_id, produit_id, lot_id, 
      quantite_theorique, code_barre, produit_nom,
      lot_numero, emplacement_theorique, statut
    )
    SELECT DISTINCT ON (p.id, l.id)
      v_tenant_id,
      p_session_id,
      p.id,
      l.id,
      COALESCE(l.quantite_restante, 0),
      COALESCE(p.code_cip, p.code_barre_externe, ''),
      p.libelle_produit,
      l.numero_lot,
      COALESCE(l.emplacement, ''),
      'non_compte'
    FROM produits p
    INNER JOIN lots l ON l.produit_id = p.id AND l.tenant_id = v_tenant_id
    WHERE p.tenant_id = v_tenant_id
      AND p.is_active = true
      AND COALESCE(l.quantite_restante, 0) > 0
      AND (l.statut IS NULL OR l.statut IN ('Disponible', 'actif'))
      AND (v_filtres_rayon IS NULL OR array_length(v_filtres_rayon, 1) IS NULL OR p.rayon_id = ANY(v_filtres_rayon))
      AND (v_filtres_fournisseur IS NULL OR array_length(v_filtres_fournisseur, 1) IS NULL OR l.fournisseur_id = ANY(v_filtres_fournisseur))
      AND (v_filtres_emplacement IS NULL OR array_length(v_filtres_emplacement, 1) IS NULL OR l.emplacement = ANY(v_filtres_emplacement));
      
  ELSIF v_type = 'cyclique' THEN
    INSERT INTO public.inventaire_items (
      tenant_id, session_id, produit_id, lot_id, 
      quantite_theorique, code_barre, produit_nom,
      lot_numero, emplacement_theorique, statut
    )
    SELECT DISTINCT ON (p.id, l.id)
      v_tenant_id,
      p_session_id,
      p.id,
      l.id,
      COALESCE(l.quantite_restante, 0),
      COALESCE(p.code_cip, p.code_barre_externe, ''),
      p.libelle_produit,
      l.numero_lot,
      COALESCE(l.emplacement, ''),
      'non_compte'
    FROM produits p
    INNER JOIN lots l ON l.produit_id = p.id AND l.tenant_id = v_tenant_id
    LEFT JOIN inventaire_items ii ON ii.produit_id = p.id 
      AND ii.tenant_id = v_tenant_id
      AND ii.created_at > NOW() - (v_cyclique_jours || ' days')::INTERVAL
    WHERE p.tenant_id = v_tenant_id
      AND p.is_active = true
      AND COALESCE(l.quantite_restante, 0) > 0
      AND (l.statut IS NULL OR l.statut IN ('Disponible', 'actif'))
      AND ii.id IS NULL
      AND (v_filtres_rayon IS NULL OR array_length(v_filtres_rayon, 1) IS NULL OR p.rayon_id = ANY(v_filtres_rayon))
      AND (v_filtres_fournisseur IS NULL OR array_length(v_filtres_fournisseur, 1) IS NULL OR l.fournisseur_id = ANY(v_filtres_fournisseur))
      AND (v_filtres_emplacement IS NULL OR array_length(v_filtres_emplacement, 1) IS NULL OR l.emplacement = ANY(v_filtres_emplacement));

  ELSIF v_type = 'peremption' THEN
    INSERT INTO public.inventaire_items (
      tenant_id, session_id, produit_id, lot_id, 
      quantite_theorique, code_barre, produit_nom,
      lot_numero, emplacement_theorique, statut
    )
    SELECT DISTINCT ON (p.id, l.id)
      v_tenant_id,
      p_session_id,
      p.id,
      l.id,
      COALESCE(l.quantite_restante, 0),
      COALESCE(p.code_cip, p.code_barre_externe, ''),
      p.libelle_produit,
      l.numero_lot,
      COALESCE(l.emplacement, ''),
      'non_compte'
    FROM produits p
    INNER JOIN lots l ON l.produit_id = p.id AND l.tenant_id = v_tenant_id
    WHERE p.tenant_id = v_tenant_id
      AND p.is_active = true
      AND COALESCE(l.quantite_restante, 0) > 0
      AND (l.statut IS NULL OR l.statut IN ('Disponible', 'actif'))
      AND l.date_peremption IS NOT NULL
      AND l.date_peremption <= NOW() + (v_filtres_peremption_jours || ' days')::INTERVAL
      AND (v_filtres_rayon IS NULL OR array_length(v_filtres_rayon, 1) IS NULL OR p.rayon_id = ANY(v_filtres_rayon))
      AND (v_filtres_fournisseur IS NULL OR array_length(v_filtres_fournisseur, 1) IS NULL OR l.fournisseur_id = ANY(v_filtres_fournisseur))
      AND (v_filtres_emplacement IS NULL OR array_length(v_filtres_emplacement, 1) IS NULL OR l.emplacement = ANY(v_filtres_emplacement));
  END IF;
  
  -- Compter les items insérés
  GET DIAGNOSTICS v_items_count = ROW_COUNT;
  
  -- Mettre à jour la session
  UPDATE public.inventaire_sessions
  SET 
    produits_total = v_items_count,
    produits_comptes = 0,
    ecarts = 0,
    progression = 0,
    statut = CASE WHEN v_items_count > 0 AND statut = 'planifie' THEN 'en_cours' ELSE statut END,
    date_debut = CASE WHEN v_items_count > 0 AND date_debut IS NULL THEN NOW() ELSE date_debut END,
    updated_at = NOW()
  WHERE id = p_session_id;
  
  RETURN jsonb_build_object(
    'success', true, 
    'inserted_count', v_items_count,
    'message', format('%s produit(s) chargé(s) pour l''inventaire', v_items_count)
  );

EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object(
    'success', false,
    'error', SQLERRM,
    'detail', SQLSTATE
  );
END;
$$;