...
-- RPC for server-side paginated movements search
...
BEGIN
  ...
  -- Count + stats on filtered set (no pagination)
  SELECT 
    COUNT(*),
    jsonb_build_object(
      'total', COUNT(*),
  ...
  -- Fetch paginated movements with joins
  SELECT COALESCE(jsonb_agg(row_data ORDER BY 
    CASE WHEN p_sort_order = 'asc' THEN
      CASE p_sort_by
        ...
        WHEN 'quantite_mouvement' THEN LPAD(ABS(m.quantite_mouvement)::text, 20, '0')
