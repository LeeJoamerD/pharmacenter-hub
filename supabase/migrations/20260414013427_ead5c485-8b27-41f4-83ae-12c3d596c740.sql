CREATE OR REPLACE FUNCTION public.search_product_by_barcode(p_barcode TEXT, p_tenant_id UUID)
RETURNS TABLE(
  ...
  lot_id UUID, numero_lot TEXT, date_peremption DATE, prix_achat_unitaire NUMERIC
)
...
BEGIN
  ...
  -- ÉTAPE 3: Retourner le produit avec le bon JOIN
  RETURN QUERY
  SELECT
    p.id, p.tenant_id,
