-- 1. Merge duplicate direct channels: keep oldest, reassign messages and participants
DO $$
DECLARE
  r RECORD;
  v_keep uuid;
BEGIN
  ...
  LOOP
    v_keep := r.ids[1];
    -- Reassign messages from duplicates to the kept channel
    UPDATE public.network_messages
    SET channel_id = v_keep
    WHERE channel_id = ANY(r.ids[2:]);
    -- Reassign participants (avoid duplicate-key collisions)
    UPDATE public.channel_participants cp
    SET channel_id = v_keep
    WHERE cp.channel_id = ANY(r.ids[2:])
      AND NOT EXISTS (
        SELECT 1 FROM public.channel_participants cp2
        WHERE cp2.channel_id = v_keep AND cp2.pharmacy_id = cp.pharmacy_id
      );
    -- Delete remaining (already-existing) duplicate participants
    DELETE FROM public.channel_participants WHERE channel_id = ANY(r.ids[2:]);
    -- Delete duplicate channels
    DELETE FROM public.network_channels WHERE id = ANY(r.ids[2:]);
  END LOOP;
END $$;

-- 2. Partial unique index to prevent future duplicates
CREATE UNIQUE INDEX IF NOT EXISTS ux_network_channels_direct_name
  ON public.network_channels (name)
  WHERE type = 'direct';

-- 3. Replace RPC with deterministic lookup (ORDER BY created_at ASC)
