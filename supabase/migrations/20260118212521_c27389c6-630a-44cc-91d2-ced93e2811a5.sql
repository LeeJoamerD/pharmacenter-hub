-- 1. Ajouter la colonne nature_compte à plan_comptable
ALTER TABLE plan_comptable 
ADD COLUMN IF NOT EXISTS nature_compte TEXT;

-- 2. Recréer la fonction avec le mapping correct
...
BEGIN
  ...
  -- Insérer avec le MAPPING CORRECT du type_compte
  INSERT INTO plan_comptable (
    tenant_id, 
    numero_compte, 
    libelle_compte, 
    type_compte,      -- niveau hiérarchique (titre, sous-titre, detail)
    nature_compte,    -- nature comptable (Actif, Passif, etc.)
    classe, 
    niveau,
    is_active, 
    analytique, 
  ...
  SELECT 
    ...
    cg.numero_compte,
    cg.libelle_compte,
    -- MAPPING : niveau -> type_compte hiérarchique
    CASE 
      WHEN cg.niveau = 1 THEN 'titre'
      WHEN cg.niveau = 2 THEN 'sous-titre'
      ELSE 'detail'
    END,
    -- Nature comptable conservée
    cg.type_compte,
    cg.classe,
  ...
  INSERT INTO parametres_plan_comptable_regionaux (
    tenant_id, classes_definition, systeme_comptable, 
    version_systeme, devise_principale
  )
  VALUES (
    p_tenant_id, v_classes, v_plan_code, 
