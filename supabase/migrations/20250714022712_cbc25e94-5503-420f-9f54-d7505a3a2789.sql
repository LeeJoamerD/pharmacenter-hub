-- Migration finale pour peupler les tables avec la bonne structure
...
-- 3. Fonction pour obtenir le tenant_id actuel pour les rôles système
DO $$
DECLARE
    current_tenant_id UUID;
BEGIN
    -- Récupérer un tenant_id existant (premier trouvé) pour les rôles système
    SELECT tenant_id INTO current_tenant_id FROM public.pharmacies LIMIT 1;
    
    -- Si aucune pharmacie n'existe, utiliser un UUID temporaire
    IF current_tenant_id IS NULL THEN
        current_tenant_id := gen_random_uuid();
    END IF;
...
-- 7. Permissions pour Préparateur
INSERT INTO public.roles_permissions (role_id, permission_id, is_active)
SELECT r.id, p.id, true
FROM public.roles r, public.permissions p
...
-- 11. Permissions pour Gestionnaire de stock
INSERT INTO public.roles_permissions (role_id, permission_id, is_active)
SELECT r.id, p.id, true
FROM public.roles r, public.permissions p
