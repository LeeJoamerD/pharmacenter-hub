...
-- 1. Fix existing data for Pharmacie Jeannelle
...
-- 2. Update clone_tenant_referential to include id_produit_source remapping
...
BEGIN
  ...
  -- Vérifier que le tenant cible n'a pas déjà de produits
  ...
  WITH ins AS (
    INSERT INTO classes_therapeutiques (tenant_id, libelle_classe, description)
    SELECT p_target_tenant, libelle_classe, description
  ...
  -- Also map by libelle for products without code_cip
  INSERT INTO _map_produits (old_id, new_id)
  SELECT s.id, t.id
  FROM produits s
