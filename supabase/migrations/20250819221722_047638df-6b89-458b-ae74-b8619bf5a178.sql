...
-- 1. Harden SECURITY DEFINER functions by adding SET search_path
...
-- 2. Create comprehensive RLS policies for sensitive tables
...
-- 4. Setup cross-tenant security triggers for all tenant tables
SELECT public.setup_cross_tenant_security_triggers();

-- 5. Update pharmacy sessions to have shorter expiry (48 hours max)
...
-- 7. Log this security hardening action
INSERT INTO public.audit_logs (
  tenant_id, action, table_name, new_values, status
) VALUES (
  NULL,
