-- Create accounting general configuration table
...
-- Create accounting numbering rules table
CREATE TABLE IF NOT EXISTS public.accounting_numbering_rules (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tenant_id UUID NOT NULL,
  rule_type TEXT NOT NULL, -- 'facture', 'ecriture', etc.
  format_pattern TEXT NOT NULL,
  current_number INTEGER NOT NULL DEFAULT 1,
  reset_frequency TEXT NOT NULL DEFAULT 'annuel', -- 'jamais', 'annuel', 'mensuel'
...
-- Create accounting currencies table
...
-- Create accounting exchange rates table
...
-- Create RLS policies for accounting_general_config
...
-- Create RLS policies for accounting_journals
...
-- Create RLS policies for accounting_numbering_rules
...
-- Create RLS policies for accounting_currencies
...
-- Create RLS policies for accounting_exchange_rates
...
-- Function to get next numbering sequence
...
-- Insert default journals for existing tenants
INSERT INTO public.accounting_journals (tenant_id, code, name, type, auto_generation)
SELECT DISTINCT p.tenant_id, 'VTE', 'Journal des Ventes', 'ventes', true FROM public.pharmacies p
ON CONFLICT (tenant_id, code) DO NOTHING;

INSERT INTO public.accounting_journals (tenant_id, code, name, type, auto_generation)
SELECT DISTINCT p.tenant_id, 'ACH', 'Journal des Achats', 'achats', true FROM public.pharmacies p
ON CONFLICT (tenant_id, code) DO NOTHING;

INSERT INTO public.accounting_journals (tenant_id, code, name, type, auto_generation)
SELECT DISTINCT p.tenant_id, 'BQ1', 'Banque Principale', 'banque', false FROM public.pharmacies p
ON CONFLICT (tenant_id, code) DO NOTHING;

INSERT INTO public.accounting_journals (tenant_id, code, name, type, auto_generation)
SELECT DISTINCT p.tenant_id, 'CAI', 'Journal de Caisse', 'caisse', false FROM public.pharmacies p
ON CONFLICT (tenant_id, code) DO NOTHING;

INSERT INTO public.accounting_journals (tenant_id, code, name, type, auto_generation)
SELECT DISTINCT p.tenant_id, 'OD', 'Opérations Diverses', 'general', false FROM public.pharmacies p
ON CONFLICT (tenant_id, code) DO NOTHING;

-- Insert default numbering rules
INSERT INTO public.accounting_numbering_rules (tenant_id, rule_type, format_pattern)
SELECT DISTINCT p.tenant_id, 'facture', 'FAC-{YYYY}-{MM}-{####}' FROM public.pharmacies p
ON CONFLICT (tenant_id, rule_type) DO NOTHING;

INSERT INTO public.accounting_numbering_rules (tenant_id, rule_type, format_pattern)
SELECT DISTINCT p.tenant_id, 'ecriture', '{JOURNAL}-{YYYY}{MM}-{####}' FROM public.pharmacies p
ON CONFLICT (tenant_id, rule_type) DO NOTHING;

-- Insert default base currency (XOF)
INSERT INTO public.accounting_currencies (tenant_id, code, name, is_base_currency)
SELECT DISTINCT p.tenant_id, 'XOF', 'Franc CFA', true FROM public.pharmacies p
ON CONFLICT (tenant_id, code) DO NOTHING;

-- Insert default exchange rate for base currency
INSERT INTO public.accounting_exchange_rates (tenant_id, currency_id, rate)
SELECT DISTINCT 
  c.tenant_id, 
  c.id, 
  1.0
