-- Create transporteurs table for delivery companies
CREATE TABLE public.transporteurs (
  ...
  delai_livraison_standard INTEGER, -- en heures
...
-- Create suivi_commandes table for order tracking
CREATE TABLE public.suivi_commandes (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tenant_id UUID NOT NULL,
  ...
  commentaire TEXT,
  agent_id UUID, -- référence au personnel qui a fait le changement
...
-- Create evaluations_fournisseurs table for supplier evaluations
CREATE TABLE public.evaluations_fournisseurs (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tenant_id UUID NOT NULL,
  ...
  evaluateur_id UUID, -- référence au personnel qui évalue
  ...
  note_globale NUMERIC(3,2) GENERATED ALWAYS AS ((note_qualite + note_delai + note_service + note_prix)::NUMERIC / 4) STORED,
  commentaires TEXT,
...
ON public.evaluations_fournisseurs 
FOR SELECT 
USING (tenant_id = get_current_user_tenant_id());
...
CREATE INDEX idx_transporteurs_tenant_id ON public.transporteurs(tenant_id);
CREATE INDEX idx_transporteurs_zone_couverture ON public.transporteurs USING GIN(zone_couverture);
CREATE INDEX idx_suivi_commandes_tenant_id ON public.suivi_commandes(tenant_id);
CREATE INDEX idx_suivi_commandes_commande_id ON public.suivi_commandes(commande_id);
CREATE INDEX idx_evaluations_fournisseurs_tenant_id ON public.evaluations_fournisseurs(tenant_id);
CREATE INDEX idx_evaluations_fournisseurs_fournisseur_id ON public.evaluations_fournisseurs(fournisseur_id);