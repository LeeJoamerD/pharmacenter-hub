...
-- Update search_cash_expenses_paginated to include total_montant in stats
...
BEGIN
  ...
  -- Count + total montant
  SELECT COUNT(*), COALESCE(SUM(mc.montant), 0)
  INTO v_total, v_total_montant
  ...
  SELECT jsonb_build_object(
    ...
    'count', v_total,
    'stats', jsonb_build_object('total_montant', v_total_montant)
  ) INTO v_result
  FROM (
    SELECT
      mc.id,
