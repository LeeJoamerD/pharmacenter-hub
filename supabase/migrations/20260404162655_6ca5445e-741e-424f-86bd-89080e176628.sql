...
BEGIN
  ...
  SELECT s.id, t.id
  FROM laboratoires s
  JOIN laboratoires t ON t.libelle = s.libelle AND t.tenant_id = p_target_tenant
  WHERE s.tenant_id = p_source_tenant;

  -- 8. Produits (schéma actuel complet avec remappage de toutes les FK)
  WITH ins AS (
    ...
    SELECT
      ...
      mf.new_id,    -- famille_id
      mr.new_id,    -- rayon_id
      mfo.new_id,   -- forme_id
      md.new_id,    -- dci_id
      mcl.new_id,   -- classe_therapeutique_id
      mc.new_id,    -- categorie_tarification_id
      ml.new_id,    -- laboratoires_id
      mr2.new_id,   -- rayon_produit_id (same rayons table)
  ...
  -- Build product mapping (by code_cip first, then by libelle)
