-- Migration corrective: Supprimer la référence à stock_alerte qui n'existe pas
  ...
  WITH stock_data AS (
    SELECT 
      ...
      CASE 
        ...
        WHEN COALESCE(SUM(l.quantite_restante), 0) <= COALESCE(p.stock_limite, 10) THEN 'alerte'
        ELSE 'normal'
      END as statut,
      COALESCE(
    ...
    GROUP BY p.id, p.libelle_produit, p.code_cip, p.prix_achat, p.stock_limite, f.libelle_famille
  )
  SELECT 
    sd.id AS produit_id,
    sd.libelle_produit,
