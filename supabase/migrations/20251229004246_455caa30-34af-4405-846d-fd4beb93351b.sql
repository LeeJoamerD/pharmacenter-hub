-- Correction de la fonction init_inventaire_items : remplacer code_ean par code_barre_externe
-- La colonne code_ean n'existe pas dans la table produits

-- Supprimer les sessions vides existantes (celles avec 0 produits comptés)
DELETE FROM public.inventaire_items WHERE session_id IN (
  SELECT id FROM public.inventaire_sessions WHERE produits_total = 0
);
DELETE FROM public.inventaire_sessions WHERE produits_total = 0;
...
BEGIN
  ...
  -- Inventaire COMPLET : tous les lots actifs
...
-- Corriger aussi la fonction de preview
