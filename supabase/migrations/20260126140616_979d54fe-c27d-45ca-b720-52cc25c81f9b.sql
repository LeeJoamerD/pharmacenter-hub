-- =====================================================
-- MIGRATION FINALE : Restauration get_pos_products et search_product_by_barcode
-- Corrige le 404 en alignant signature frontend/backend
-- =====================================================

-- 1) Supprimer TOUTES les variantes existantes pour éviter les conflits
...
-- 2) Recréer get_pos_products avec signature frontend (p_page_size, p_page) et RETURNS json
...
BEGIN
  ...
  -- Compter le total des produits correspondants
  ...
  -- Récupérer les produits avec leurs infos stock
  ...
  FROM (
    SELECT 
      ...
      -- Prix depuis produits (source de vérité)
      ...
      -- Catégorie via famille_produit
      COALESCE(f.libelle_famille, 'Non catégorisé') AS category,
      -- Stock disponible (lots valides : date NULL ou future)
      COALESCE(
      ...
      -- Plus proche date de péremption (ignorer NULL)
      ...
      -- Tous les lots sont expirés ? (seulement si aucun lot valide)
      NOT EXISTS(
        SELECT 1 FROM lots l 
        WHERE l.produit_id = p.id 
          AND l.quantite_restante > 0
  ...
  -- Retourner le JSON structuré attendu par le frontend
...
-- 3) Recréer search_product_by_barcode avec RETURNS json
...
BEGIN
  ...
  FROM (
    ...
    WHERE p.tenant_id = p_tenant_id
      AND p.is_active = true
      AND (p.code_cip = p_barcode OR p.code_ean = p_barcode)
    LIMIT 1
  ) AS product_data;
...
-- 4) Permissions pour authenticated et anon
