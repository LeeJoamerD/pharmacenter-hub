-- RPC pour récupérer les alertes de stock avec logique de cascade et pagination
-- Traite TOUS les produits (6523) côté serveur avec calcul des seuils en cascade
...
BEGIN
  ...
  v_query := format('
    WITH stock_data AS (
      SELECT 
        ...
        MAX(l.date_derniere_entree) as dernier_mouvement
      FROM produits p
  ...
  v_count_query := format('
    WITH stock_data AS (
      ...
      LEFT JOIN lots l ON l.produit_id = p.id AND l.tenant_id = p.tenant_id
      LEFT JOIN global_alert_settings gas ON gas.tenant_id = p.tenant_id
      WHERE p.tenant_id = %L AND p.actif = true
...
COMMENT ON FUNCTION get_stock_alerts_with_products IS 'Récupère les produits en alerte (rupture/critique/faible) avec logique de cascade pour les seuils (Produit > Settings > Défaut) et pagination côté serveur pour traiter tous les 6523 produits';