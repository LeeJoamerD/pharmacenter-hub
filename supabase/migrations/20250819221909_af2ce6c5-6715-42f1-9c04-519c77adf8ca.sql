-- SECURITY FIX: Comprehensive security hardening (Final version)

-- 1. Remove foreign key constraint on audit_logs.tenant_id to allow system events
...
BEGIN
  ...
  -- Log monitoring execution (no tenant_id for system events)
  INSERT INTO public.audit_logs (
    user_id, action, table_name, new_values, status
  ) VALUES (
    NULL, 
    'SECURITY_MONITORING',
    'system',
...
-- 8. Log this security hardening action (system event without tenant)
INSERT INTO public.audit_logs (
  user_id, action, table_name, new_values, status
) VALUES (
  NULL,
  ...
  jsonb_build_object(
    ...
    'changes', ARRAY[
      ...
      'removed_tenant_fk_constraint_for_system_events'
    ]
  ),
  'success'
);