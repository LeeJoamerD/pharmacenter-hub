...
-- Migrer les données existantes de resolved vers status
...
-- Supprimer la colonne resolved (redondante)
ALTER TABLE public.security_alerts 
...
-- Supprimer la colonne resolved si elle existe (redondante avec status)
ALTER TABLE public.security_incidents 
DROP COLUMN IF EXISTS resolved;
