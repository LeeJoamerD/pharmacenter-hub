-- ============================================
-- CORRECTION COMPLÈTE SECTION ANALYTIQUE
-- ============================================

-- 1. Supprimer d'abord la fonction existante get_profitability_data
DROP FUNCTION IF EXISTS get_profitability_data(UUID, DATE, DATE, UUID, INT);

-- 2. Recréer la RPC get_profitability_data avec les bons types
...
-- 3. Créer une RPC pour les métriques globales de rentabilité (sans limite de pagination)
...
BEGIN
  ...
  INTO v_ca_total, v_cout_total, v_marge_totale, v_taux_marge_global, v_nb_produits_vendus
...
-- 4. Corriger get_analytical_summary pour calculer la marge correctement
CREATE OR REPLACE FUNCTION get_analytical_summary(p_tenant_id UUID)
RETURNS JSON AS $$
DECLARE
  v_result JSON;
  v_nb_centres INT;
  ...
  v_cout_total NUMERIC;
BEGIN
  -- Compter les centres de coûts actifs
  SELECT COUNT(*) INTO v_nb_centres
  FROM centres_couts 
  WHERE tenant_id = p_tenant_id AND est_actif = true;

  -- Calculer budget et réalisé
  SELECT 
    COALESCE(SUM(montant_prevu), 0),
    COALESCE(SUM(montant_realise), 0)
  INTO v_budget_total, v_realise_total
  FROM budgets 
  WHERE tenant_id = p_tenant_id;

  -- Calculer la marge globale directement à partir des ventes
  SELECT 
    COALESCE(SUM(lv.montant_ligne_ttc), 0),
    COALESCE(SUM(lv.quantite * p.prix_achat), 0)
  INTO v_ca_total, v_cout_total
  FROM lignes_ventes lv
  JOIN ventes v ON lv.vente_id = v.id
  JOIN produits p ON lv.produit_id = p.id
  WHERE p.tenant_id = p_tenant_id;

  IF v_ca_total > 0 THEN
    v_marge_globale := ROUND((v_ca_total - v_cout_total) / v_ca_total * 100, 2);
  ELSE
    v_marge_globale := 0;
  END IF;

  v_result := json_build_object(
    'nbCentres', v_nb_centres,
    'budgetTotal', v_budget_total,
    'realiseTotal', v_realise_total,
    'margeGlobale', v_marge_globale,
    'caTotal', v_ca_total,
    'coutTotal', v_cout_total
  );
...
GRANT EXECUTE ON FUNCTION get_analytical_summary(UUID) TO authenticated;