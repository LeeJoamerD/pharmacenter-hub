...
BEGIN
  ...
  -- FIXED: Replace stock_alerte with stock_critique and stock_faible (actual schema columns)
  SELECT COUNT(*) INTO v_products_with_stock
  FROM produits
  WHERE tenant_id = p_tenant_id 
    AND is_active = true 
    AND (stock_limite IS NOT NULL OR stock_critique IS NOT NULL OR stock_faible IS NOT NULL);
