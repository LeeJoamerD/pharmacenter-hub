-- =============================================
-- FIX: Ignorer les dates de péremption NULL
-- Les produits sans date de péremption ne doivent pas être bloqués
-- =============================================

-- 1. Recréer get_pos_products avec gestion NULL
...
BEGIN
  ...
  FROM (
    SELECT 
      ...
      -- Date expiration la plus proche (lot avec stock, exclut NULL)
      (
      ...
      -- Y a-t-il au moins un lot valide (non expiré OU sans date, avec stock)?
      EXISTS(
      ...
      -- Tous les lots sont-ils expirés? (false si au moins un lot valide ou sans date)
      (
        COALESCE((
          SELECT SUM(l.quantite_restante)
          FROM lots l
...
-- 2. Recréer search_product_by_barcode avec gestion NULL
...
BEGIN
  ...
  FROM (
    SELECT 
      ...
      -- Date expiration la plus proche (exclut NULL)
      (
        SELECT MIN(l.date_peremption)
        FROM lots l
        WHERE l.produit_id = p.id 
      ...
      -- Au moins un lot valide (non expiré OU sans date)?
      EXISTS(
        SELECT 1 FROM lots l
        WHERE l.produit_id = p.id 
          AND l.tenant_id = p_tenant_id
      ...
      -- Tous lots expirés? (false si au moins un lot valide ou sans date)
      (
        COALESCE((
          SELECT SUM(l.quantite_restante)
          FROM lots l
