-- Supprimer l'ancienne version de la fonction (1 paramètre uniquement)
DROP FUNCTION IF EXISTS public.generate_piece_number(UUID);

-- Supprimer la version avec 2 paramètres aussi pour éviter les conflits
DROP FUNCTION IF EXISTS public.generate_piece_number(UUID, DATE);

-- Recréer la fonction corrigée utilisant journaux_comptables
...
GRANT EXECUTE ON FUNCTION public.generate_piece_number(UUID, DATE) TO authenticated;