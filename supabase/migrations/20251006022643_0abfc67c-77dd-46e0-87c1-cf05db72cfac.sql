-- Fix the generer_alertes_expiration_automatiques function to handle partial unique index
-- The ON CONFLICT clause must match the partial index condition
...
BEGIN
  ...
  LOOP
    ...
    IF jours_restants <= param_record.delai_alerte_jours THEN
      ...
      -- Créer ou mettre à jour l'alerte avec la condition WHERE dans ON CONFLICT
