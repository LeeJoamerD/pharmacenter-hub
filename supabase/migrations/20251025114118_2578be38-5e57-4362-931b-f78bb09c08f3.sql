-- Migration corrective : Restaurer tenant_id et fonction register_pharmacy_with_admin

-- Partie 1 : Restaurer tenant_id dans pharmacies
...
-- Rendre tenant_id obligatoire
ALTER TABLE public.pharmacies 
ALTER COLUMN tenant_id SET NOT NULL;

-- Contrainte : pharmacie est son propre tenant
ALTER TABLE public.pharmacies 
DROP CONSTRAINT IF EXISTS pharmacies_tenant_id_equals_id;

ALTER TABLE public.pharmacies 
ADD CONSTRAINT pharmacies_tenant_id_equals_id 
CHECK (tenant_id = id);

-- Partie 2 : Recréer la fonction register_pharmacy_with_admin avec la bonne signature
...
BEGIN
  ...
  -- 1. Créer la pharmacie avec tenant_id = id
  INSERT INTO public.pharmacies (
    id, tenant_id, nom_pharmacie, numero_agrement, adresse, 
    ville, pays, email, telephone_appel, telephone_whatsapp,
    created_at, updated_at
  ) VALUES (
    new_pharmacy_id,
    new_pharmacy_id,
    pharmacy_data->>'name',
    COALESCE(pharmacy_data->>'licence_number', 'LIC-' || substr(new_pharmacy_id::text, 1, 8)),
    pharmacy_data->>'address',
    pharmacy_data->>'city',
    COALESCE(pharmacy_data->>'pays', 'Bénin'),
    pharmacy_data->>'email',
