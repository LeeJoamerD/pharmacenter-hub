-- Complete rewrite of search_lots_paginated to match actual schema
-- Fixes: p.nom -> p.libelle_produit, p.code_barre -> p.code_barre_externe, 
-- l.date_entree -> COALESCE(l.date_reception, l.created_at), produit_detail as JSON array
...
BEGIN
  ...
  -- Get paginated results with proper column mappings
  EXECUTE format('
  ...
  INTO v_result
  USING p_tenant_id, p_search, p_limit, v_total_count, p_sort_by, v_offset;
