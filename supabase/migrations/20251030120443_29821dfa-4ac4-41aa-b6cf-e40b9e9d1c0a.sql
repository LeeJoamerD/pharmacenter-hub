-- ============================================================================
-- PHASE 1: DROP DES FONCTIONS RPC EXISTANTES
-- ============================================================================

DROP FUNCTION IF EXISTS public.rpc_stock_record_movement(uuid, uuid, text, integer, numeric, uuid, text, text, uuid, uuid, text, text, text, jsonb);
DROP FUNCTION IF EXISTS public.rpc_stock_update_movement(uuid, integer, text, text, jsonb);
DROP FUNCTION IF EXISTS public.rpc_stock_delete_movement(uuid);

-- ============================================================================
-- PHASE 2: CRÉATION DES FONCTIONS RPC AVEC SIGNATURES CORRECTES
-- ============================================================================

-- 2.1: Créer rpc_stock_record_movement avec signature complète (14 paramètres)
...
BEGIN
  ...
  -- Récupération des paramètres de stock
  SELECT * INTO stock_settings
  FROM public.stock_settings
  WHERE tenant_id = current_tenant_id
  LIMIT 1;

  -- Si aucun paramètre, utiliser les valeurs par défaut
  ...
  -- Gestion du transfert vers lot destination
  ...
  -- Insertion dans stock_mouvements (table globale)
...
-- 2.2: Créer rpc_stock_update_movement avec paramètre p_movement_id
CREATE FUNCTION public.rpc_stock_update_movement(
  p_movement_id UUID,
...
BEGIN
  ...
  IF p_quantite_mouvement IS NOT NULL AND p_quantite_mouvement != movement_record.quantite_mouvement THEN
    ...
    -- Calculer la nouvelle quantité après
...
-- 2.3: Créer rpc_stock_delete_movement avec paramètre p_movement_id
CREATE FUNCTION public.rpc_stock_delete_movement(
  p_movement_id UUID
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Calculer la quantité après annulation
  CASE movement_record.type_mouvement
    WHEN 'sortie', 'destruction' THEN
  ...
  -- Supprimer aussi de stock_mouvements si présent
...
-- ============================================================================
-- PHASE 3: AMÉLIORATION DES RLS POLICIES
-- ============================================================================

-- Policy UPDATE pour mouvements_lots
...
-- Policy DELETE pour mouvements_lots
