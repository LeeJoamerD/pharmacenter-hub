...
BEGIN
  -- Normaliser le code-barres : remplacer ° par - (scanner physique vs DB)
  v_barcode := REPLACE(p_barcode, '°', '-');

  -- ============================================
  -- ÉTAPE 1: Recherche PRIORITAIRE par code-barres LOT
  -- D'abord correspondance exacte, puis par préfixe
  -- ============================================
  
  -- 1a: Correspondance exacte sur code_barre
  SELECT 
    l.id AS lot_id,
    l.produit_id,
    l.numero_lot,
  ...
  -- 1b: Correspondance exacte sur numero_lot
  ...
  -- 1c: Correspondance par préfixe (code-barres scanné est préfixe ou extension)
  IF NOT v_found_via_lot THEN
    SELECT 
      l.id AS lot_id,
      l.produit_id,
    ...
    ORDER BY 
      -- Priorité : correspondance la plus longue d'abord
      GREATEST(
  ...
  -- ============================================
  -- ÉTAPE 2: Recherche classique par code produit (si pas trouvé via lot)
  -- ============================================
  IF NOT v_found_via_lot THEN
    ...
    WHERE p.tenant_id = p_tenant_id
      ...
      AND (
        ...
        OR p.code_ean = v_barcode
        OR p.code_interne = v_barcode
