...
BEGIN
  ...
  IF current_tenant_id IS NULL THEN
    RAISE EXCEPTION 'Tenant non trouvé pour l''utilisateur courant';
  END IF;
...
-- 2. Création de get_low_stock_products
...
BEGIN
  ...
  -- CTE pour le calcul du stock faible
  WITH stock_data AS (
    SELECT 
      p.id,
      p.tenant_id,
  ...
  filtered_data AS (
    ...
    WHERE 
      (p_search IS NULL OR 
       sd.code_cip ILIKE '%' || p_search || '%' OR 
       sd.libelle_produit ILIKE '%' || p_search || '%')
      AND (p_category IS NULL OR sd.famille_id = p_category)
      AND (p_status IS NULL OR sd.statut_stock = p_status)
  ),
  ...
  SELECT COUNT(*) INTO total_count FROM filtered_data;
