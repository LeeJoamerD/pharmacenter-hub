...
-- Créer la vue détaillée des mouvements
CREATE OR REPLACE VIEW public.v_mouvements_lots_details AS
SELECT 
  ml.*,
  l.numero_lot,
  l.date_peremption,
  l.quantite_restante as lot_quantite_restante,
  l.emplacement_source,
  l.emplacement_destination,
  p.libelle_produit,
  p.code_cip
FROM public.mouvements_lots ml
LEFT JOIN public.lots l ON ml.lot_id = l.id
LEFT JOIN public.produits p ON ml.produit_id = p.id;
