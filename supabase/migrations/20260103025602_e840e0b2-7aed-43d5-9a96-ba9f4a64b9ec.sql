-- Ajouter les colonnes manquantes dans clients
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    AND column_name = 'taux_ticket_moderateur'
  ) THEN
    ALTER TABLE public.clients ADD COLUMN taux_ticket_moderateur numeric DEFAULT 0;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public' 
    AND table_name = 'clients' 
    AND column_name = 'caution'
  ) THEN
    ALTER TABLE public.clients ADD COLUMN caution numeric DEFAULT 0;
  END IF;
END $$;

-- ==============================================
-- FONCTIONS POUR PERSONNEL (corrigées avec bon enum)
-- ==============================================
...
-- ==============================================
-- FONCTIONS POUR SOCIETES (corrigées avec bon enum)
-- ==============================================
