...
-- Update existing statut check constraint if it exists
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'inventaire_sessions_statut_check' 
    AND table_name = 'inventaire_sessions'
  ) THEN
    ALTER TABLE public.inventaire_sessions DROP CONSTRAINT inventaire_sessions_statut_check;
  END IF;
END $$;
...
-- Create trigger for inventory lines updated_at
