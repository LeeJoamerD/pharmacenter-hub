    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          prix_achat: sourceProduct.prix_achat ? Math.round((sourceProduct.prix_achat / quantity) * 100) / 100 : null,
          prix_vente_ht: sourceProduct.prix_vente_ht ? Math.round((sourceProduct.prix_vente_ht / quantity) * 100) / 100 : null,
          prix_vente_ttc: sourceProduct.prix_vente_ttc ? Math.round((sourceProduct.prix_vente_ttc / quantity) * 100) / 100 : null,
          stock_critique: (sourceProduct as any).stock_critique,
          stock_faible: (sourceProduct as any).stock_faible,
          stock_limite: (sourceProduct as any).stock_limite,
          is_active: true,
          id_produit_source: sourceProduct.id,
          quantite_unites_details_source: quantity,
          niveau_detail: newLevel
